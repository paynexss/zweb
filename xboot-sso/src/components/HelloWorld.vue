<template>
  <div class="hello">
    ...
    <RadioGroup v-model="mode" style="margin-top:30px;">
      <Radio label="1">
        <span>共享Cookie模式</span>
      </Radio>
      <Radio label="2">
        <span>OAuth2.0模式</span>
      </Radio>
    </RadioGroup>
    ...
    <div v-show="mode=='2'">
      <div v-if="login2">
        ...
        <br />
        <p>用户名：{{userInfo.username}}</p>
        <Button @click="login2=false" type="primary" style="margin-top:10px;margin-right:10px;">退出登录</Button>
        <a href="http://xboot.exrick.cn/login" target="_blank">
          <Button style="margin-top:10px;">返回主站点</Button>
        </a>
      </div>
      ...
      <div v-else style="margin-top:50px;">
        <Button @click="toAuthorize" type="primary" v-if="!loading" style="margin-top:10px;">开始认证</Button>
        <ScaleLoader :height="38" :width="5" :radius="0" color="#57a3f3" v-if="loading" />
      </div>
    </div>

    <div class="footer">
      <h3>本站源码获取</h3>
      <span>本站源码可通过XBoot完整版获取</span>
  ...
  data() {
    return {
      ...
      form: {
        ...
        client_id: "211260981215498241",
        client_secret: "147089e7b62142b4a5cf2d5d979ffee1",
        redirect_uri: "http://sso.exrick.cn"
      }
    };
  },
  methods: {
    ...
    toAuthorize() {
      window.location.href =
        "http://xboot.exrick.cn/authorize?client_id=" +
        this.form.client_id +
        "&redirect_uri=" +
    ...
    oauth2() {
      this.loading = true;
      // 获取token
      this.getRequestWithNoToken("/oauth2/token", this.form).then(res => {
        if (res.success) {
          // 获取用户信息
          this.getRequestWithNoToken("/oauth2/user", {
